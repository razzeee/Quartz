...
function Enemy:UNIT_CASTEVENT()
	...
	if eventType == "START" or eventType == "CHANNEL" then
		if not casts[caster] then
			casts[caster] = new()
		end
		local sName = UnitName(caster)
		local spellName, rank, texture =  SpellInfo(spellId)
		casts[caster].name = sName
		casts[caster].spellName = spellName
		casts[caster].spellId = spellId
		casts[caster].texture = texture
		casts[caster].duration = duration
		casts[caster].startTime = start
...
do
	local function onUpdate()
		...
		if currentTime > endTime then
			...
			bar.TimeText:SetText(format(TimeFmt(endTime - currentTime)))
		end
	end
	...
	function Enemy:UpdateBars()
		...
		for i = getn(tmp)+1, getn(castbars) do
			castbars[i]:Hide()
		end
		del(tmp)
	end
							...
							lock = {
								...
								set = function(info, v)
									if v then
										...
										castbars[1]:SetMinMaxValues(0,1)
										castbars[1]:SetValue(1)
										castbars[1]:Show()
